...
class Tree(Plant):
    def __init__(self, name: str, height: int, age: int, trunck: int, shade_long: int , shade_wide : int):
        super().__init__(name, height, age)
    ...
    def produce_shade(self, long: int, wide: int)-> None :
        ...
        print(f"Tree {self._name} now produces a shade of {round(self._shade_long, 1)}cm and {round(self._shade_wide, 1)}cm wide.")

class Vegetable(Plant):
    def __init__(self, name: str, height: int, age: int, harvest: str, nutri: int):
        super().__init__(name, height, age)
